...
class PayoutsProvider extends ChangeNotifier {
  PayoutsProvider() {
    contact = SharedPrefsUtil().getString('firebase')!;
  ...
  late String contact;
  final uid = SharedPrefsUtil().getString('uid')!;
  bool isLoading = true;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (Map payout in temp) {
          if (payout['accountNumber'] == contact && payout['type'] == "rider") {
            latestPayout ??= payout;
  ...
  void requestPayout(BuildContext context) async {
    ...
    final earningData = await Navigator.of(context)
    ...
    if (earningData != null) {
      ...
      try {
        final response = await http.post(Uri.parse("$baseurl/payoutrequest"),
            headers: {"Content-Type": "application/json"},
            body: jsonEncode({
  ...
  static FutureOr updateEarnings(Map<String, dynamic> message) async {
    ...
    for (var earning in earnings) {
      await EarningsController.updateEarning(
          uid: uid, earningId: earning, newStatus: 'completed');
    }
  }
}
  ...
  void getEarnings() async {
    try {
      ...
      if (response.containsKey("earnings")) {
        List allEarnings = response['earnings']['earnings'];
  ...
  Widget build(BuildContext context) {
    return DecoratedBox(
      ...
      child: Scaffold(
        ...
        floatingActionButton: earnings.isEmpty
            ? null
